...
export class CodexAppServer {
  ...
  async run({cwd,job,signal,progress=()=>{},approach='',model=null,effort=null,conversation=false,sessionId=null,onThread=()=>{},contextTool}) {
    ...
    const started=sessionId?(this.loadedThreads.has(sessionId)?{thread:{id:sessionId}}:await this.rpc('thread/resume',{threadId:sessionId,cwd,sandbox:conversation?'read-only':'workspace-write',approvalPolicy:'never',model,excludeTurns:true})):await this.rpc('thread/start',{cwd,sandbox:conversation?'read-only':'workspace-write',approvalPolicy:'never',model,...(this.projectId?{projectId:this.projectId}:{}),...(contextTool?{dynamicTools:contextTools}:{})});
