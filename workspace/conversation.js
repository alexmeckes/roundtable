...
export function createConversation({say,announce,allowRun}) {
  ...
  function drain(room,bridge){
    ...
    const queue=queues.get(identity(bridge)),next=queue?.shift();if(!next)return;
    if(!allowRun(room)){queues.delete(identity(bridge));return;}
  ...
  function enqueue(room,bridge,text,chain){
    ...
    const queue=queues.get(identity(bridge))||[];if(queue.length>=8){say(room,{kind:'system',text:label(bridge)+' has a full reply queue. Please ask again shortly.'});return;}
  ...
  function result(ws,room,msg){
    ...
    if(!enabled(room).includes(bridge)){queues.delete(identity(bridge));announce(room);return;}
